...
3. How Agent Auth capability tokens bind to De-pin payment identity? → **Proposed answer:** capability claims `stellar_pk` + `allowed_payer_pks` + `pay_mode`; edge checks x402 **payer** ∈ allowlist (not `payTo`). See `docs/agent_auth_implementation_plan.md` § Stellar.
