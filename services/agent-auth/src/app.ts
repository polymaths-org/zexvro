...
export function createGateApp(config: GateConfig, repo: GateRepository) {
  ...
  if (!config.isProd) {
    app.get('/v1/admin/demo-keys', adminAuth, async (_req, res) => {
      ...
      res.json({
        ...
        note: 'Local/dev only. Never use demo secrets in production.',
      })
    })
  }
  ...
  // --- Local-only surfaces (never registered when NODE_ENV=production) ---
  // Curate tooling, sample merchant pages, and captcha playground stay off production.
  if (!config.isProd) {
    ...
    app.get('/demo/curate/api/candidates', (req, res) => {
      ...
      try {
        ...
        res
          .status(500)
          .json(problem(500, 'curate_failed', e instanceof Error ? e.message : 'failed'))
      }
    })
    ...
    app.post('/demo/curate/api/save', (req, res) => {
      ...
      try {
        ...
        res
          .status(500)
          .json(problem(500, 'save_failed', e instanceof Error ? e.message : 'failed'))
      }
    })
    ...
    app.post('/demo/site/api/checkout', async (req, res) => {
      const capability =
        ...
        (typeof req.header('x-zexvro-capability') === 'string' &&
          req.header('x-zexvro-capability')) ||
        ''
  ...
  /** Human captcha feedback (broken image / unclear challenge / accessibility). */
