...
- Files: `docs/agent_auth_local_demo.md` (single demo walkthrough), slimmed docs (removed plan/critique bloat), `scripts/gate-local-demo.mjs`, demo routes `/demo/site` + `/demo/captcha`, `gate:agent-site-demo`, optional `gate:captcha-llm-demo` (human-channel bot QA only).
- Summary: Gate is demoable fully **locally** for another person: human captcha ceremony, autonomous agent crypto path, optional vision-LLM human-bot QA via env keys. Third-party developers protect actions with captcha (humans) or register keys (agents). **No AWS deploy** without explicit approval.
- Decisions:
  - **Accepted** — Humans use captcha UI; agents never use captcha (Ed25519 + PoP only).
  - **Accepted** — Captcha LLM script is product QA for human channel, not the agent demo.
  - **Accepted** — Curate UI (`/demo/curate`) is operator tooling, not product marketing surface.
  - **Accepted** — Keep main free of plan dumps / session memory bloat; product docs = quickstart + local demo + captcha + developer guide.
- Follow-ups: Owner local test → optional PR without curate/deploy bloat; AWS only when approved.
- Blockers: None for local demo if Gate runs on :4103. Captcha photo bank is local/gitignored (`captcha-assets/verified`).
- Verification: `npm run gate:demo` (checklist + health); `npm run gate:agent-site-demo`; open `/demo/site` and `/demo/captcha`.
