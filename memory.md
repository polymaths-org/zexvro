...
## 2026-07-17 — Agent Auth / ZEXVRO Gate: research + implementation plan + Phase 1 scaffold

- Author: Codex (Rushi / Wraient ownership area)
- Service: Captcha-like Agent Authentication Service (product name **Proposed**: ZEXVRO Gate)
- Summary: Market + security research completed; dual-channel anti-relay architecture written; Phase 1 API scaffold with tests.
- Files:
  - `docs/agent_auth_implementation_plan.md` (full production plan)
  - `services/agent-auth/**` (Express API: challenges, verify, agent registry, demo keys)
  - `packages/agent-auth-sdk/**` (`@zexvro/gate` client stubs)
  - `packages/agent-auth-types/**` (shared header/claim constants)
  - root `package.json` scripts `*:agent-auth`
  - root `.env.example` Agent Auth vars
- Decisions:
  - **Proposed** — Product is a dual-channel **capability gate**, not a CAPTCHA; agents are first-class.
  - **Proposed** — Anti-relay: class set only by ceremony channel; human cannot mint agent capabilities and vice versa; bind action/origin/site/client key; short TTL + jti.
  - **Proposed** — Ship name **ZEXVRO Gate**; dashboard route may remain `/agent-auth`.
  - **Proposed** — Header `X-Zexvro-Capability`.
  - **Proposed** — Stellar MVP: agent `G…`/ed25519 registry + capability claims `stellar_pk` / `pay_mode` / `allowed_payer_pks` for De-pin **payer** bind (not payTo; not "wallet = human").
  - **Proposed** — Human v1 prioritizes silent/soft + WebAuthn later; no image puzzles; no HDM in MVP; no classifier inside De-pin.
  - **Accepted** (existing) — Coordinate Access Shield with Nabil; identity ≠ payment.
- Verification: `cd services/agent-auth && npm test` — 5/5 passing (after npm cache clean freed disk).
- Follow-ups:
  - WebAuthn hard step-up; Cognito admin routes; Dynamo + App Runner deploy.
  - Wire `frontend/src/components/services/AgentAuth.tsx` to real Keys/Policies/Agents/Events.
  - De-pin optional middleware for capability + payer allowlist (with Nabil).
  - Promote Proposed decisions to Accepted in `context.md` when team agrees on Gate naming.
- Blockers: Disk was 100% full during first `npm install` (resolved via `npm cache clean`); keep an eye on disk before large installs.
- Agent-safe notes: Do not claim perfect detection; do not add puzzle CAPTCHAs; do not fork classifier into De-pin.

## 2026-07-17 — Agent Auth follow-up: dashboard IA, smoke, docs cross-links

- Author: Codex (Rushi area)
- Service: ZEXVRO Gate / Agent Auth
- Summary: Dashboard stub reframed from CAPTCHA mock to Keys/Policies/Agents/Events; live smoke script; context + Access Shield cross-links.
- Files:
  - `frontend/src/components/services/AgentAuth.tsx`
  - `frontend/src/data/serviceCatalog.ts`, `frontend/src/data/mock.ts`
  - `scripts/agent-auth-smoke.mjs` + root `agent-auth:smoke`
  - `docs/agent_auth_privacy.md`, `docs/agent_auth_openapi.sketch.yaml`
  - `services/agent-auth/Dockerfile`
  - `context.md` (Agent Auth unknowns → Proposed pointers)
  - `docs/access_shield.md` Q3 Proposed payer-bind answer
- Verification: `npm --prefix services/agent-auth test` (7/7); live smoke human+agent verify OK on :4103.
- Follow-ups: Cognito-protect admin routes; WebAuthn; Dynamo; App Runner deploy; De-pin middleware with Nabil; remove demo-keys from non-dev.
- Blockers: None for design; disk pressure fixed earlier via npm cache clean.

## 2026-07-17 — Gate plan adversarial reconcile (v0.2)

- Author: Codex (Rushi)
- Service: ZEXVRO Gate / Agent Auth
- Summary: Hostile review of Phase-1 plan. Accepted honesty: soft_confirm is not security; PoP not enforced yet; jti now use-budget; prod kills soft/HMAC; exact act match; ADR-002 freezes.
- Files:
  - `docs/adr-002-gate-protocol-v0.2.md` (reconciled freezes)
  - `docs/agent_auth_implementation_plan.md` (banner → ADR-002)
  - `services/agent-auth` (prod flags, reuse budget, origin required, status honesty)
- Decisions:
  - **Accepted** — Do not claim soft_confirm anti-relay or farm-stop.
  - **Accepted** — Agent path is the production-grade v1 bet; human hard = WebAuthn later (M3).
  - **Accepted** — DX: one package mental model; hide claim catalog.
  - **Accepted** — jti presentation budget (consume on verify).
  - **Accepted** — `act` exact match only.
  - **Proposed** — PoP mandatory before GA (M1 incomplete until done).
- Verification: `npm --prefix services/agent-auth test` — 17 tests green including production soft_confirm rejection and jti exhaustion.
- Follow-ups: Ed25519 fixture tests without relying on HMAC; implement PoP; WebAuthn; Dynamo; De-pin payer enforce with Nabil.
- Blockers: None for design direction; PoP remains engineering debt before security marketing.

## 2026-07-17 — Gate: session_pop human path + Dynamo codec + middleware

- Author: Codex (Rushi)
- Service: ZEXVRO Gate
- Summary: Production-allowed human `session_pop` (Ed25519 key possession); Dynamo single-table codec; Express `gateMiddleware`; UI/ops docs. Tests 21 green.
- Files:
  - `services/agent-auth/src/challenges.ts` (session_pop)
  - `services/agent-auth/src/stores.dynamo.ts` + tests
  - `packages/agent-auth-sdk/src/middleware.js`
  - `docs/agent_auth_production_ops.md`, `docs/agent_auth_ui_spec.md`
- Decisions:
  - **Accepted** — `session_pop` is the production *soft* human path (anti-relay vs string CAPTCHA; not WebAuthn-grade).
  - **Accepted** — soft_confirm remains dev-only.
  - **Proposed** — Dynamo table name `zexvro-agent-auth` single-table pk/sk + TTL.
- Verification: `npm --prefix services/agent-auth test` (21).
- Follow-ups: WebAuthn; wire AWS Dynamo DocumentClient; App Runner deploy; browser WebCrypto session_pop helper.

## 2026-07-17 — Gate AWS table/secret + browser session_pop widget

- Author: Codex (Rushi)
- Service: ZEXVRO Gate
- Summary: Created AWS DynamoDB `zexvro-agent-auth` (ACTIVE, TTL on) and Secrets Manager `zexvro/agent-auth/issuer-signing`. Browser WebCrypto session_pop helper + GateProtect UI widget. Deploy script hardened with waiter.
- Files:
  - AWS: table + secret (account 290294660486 us-east-1)
  - `packages/agent-auth-sdk/src/browser.js`
  - `frontend/src/components/gate/GateProtect.tsx` + AgentAuth overview embed
  - `scripts/deploy-agent-auth-aws.mjs`
- Decisions:
  - **Accepted** — Dynamo table name `zexvro-agent-auth` pk/sk + ttl
  - **Accepted** — Signing secret path `zexvro/agent-auth/issuer-signing` (rotate before prod traffic)
- Verification: `npm --prefix services/agent-auth test` (21); aws describe-table ACTIVE; secret ARN present.
- Follow-ups: Wire DocumentClient into service runtime; App Runner image; WebAuthn; rotate secret.
- Blockers: App Runner image build not run (disk/cost); secret value is placeholder.

## 2026-07-17 — Gate Dynamo runtime repository wired + live smoke

- Author: Codex (Rushi)
- Service: ZEXVRO Gate
- Summary: Introduced `GateRepository` with Memory + Dynamo implementations. Server selects backend via `GATE_STATE_BACKEND=memory|dynamo`. Live AWS smoke: jti budget 2→exhausted, agent put/get, demo site bootstrap.
- Files:
  - `services/agent-auth/src/repository.ts`
  - `services/agent-auth/src/repository.dynamo.ts`
  - `services/agent-auth/src/{tokens,challenges,verify,app,server}.ts` (async repo)
  - deps: `@aws-sdk/client-dynamodb`, `@aws-sdk/lib-dynamodb`
  - `scripts/gate-dynamo-smoke.mjs`
- Decisions:
  - **Accepted** — Single-table Dynamo for challenges/jti/agents; site config cached after seed.
  - **Accepted** — Default still `memory` for local tests; production uses `GATE_STATE_BACKEND=dynamo`.
- Verification: unit tests 21/21; `node scripts/gate-dynamo-smoke.mjs` → consume [true,true,false], agentFound true.
- Follow-ups: App Runner service + IAM role; WebAuthn; rotate signing secret; Cognito admin.
- Blockers: App Runner image not deployed yet.

## 2026-07-17 02:01 — Gate WebAuthn scaffold + session end plan delivery

- Author: Codex (Rushi)
- Service: ZEXVRO Gate
- Summary: Past 2:00 IST target. Added WebAuthn options scaffold (`webauthn.ts`) with honest not-implemented verify; 23 tests green. Dynamo runtime + AWS table/secret already live. Delivering consolidated implementation plan to user.
- Files: `services/agent-auth/src/webauthn.ts`, `webauthn.test.ts`, `app.ts` human tasks include webauthn_get scaffold.
- Decisions: **Accepted** — WebAuthn verify deferred to M3 with @simplewebauthn/server; scaffold documents RP binding requirements.
- Verification: npm test 23/23; lint clean; Dynamo ACTIVE.
- Follow-ups: App Runner deploy; wire simplewebauthn; Cognito admin; De-pin payer middleware; rotate signing secret.
- Blockers: None for plan delivery; GA still open.

## 2026-07-17 02:02 — Gate developer guide + Dockerfile prod defaults

- Author: Codex (Rushi)
- Service: ZEXVRO Gate
- Summary: Past 2:00 IST. Added DEVELOPER_GUIDE one-pager; Dockerfile production defaults. Docker daemon unavailable for local image build. Tests 23/23.
- Files: `docs/agent_auth_DEVELOPER_GUIDE.md`, `services/agent-auth/Dockerfile`
- Follow-ups: Start docker.service for image build; App Runner; WebAuthn verify; rotate secret.
- Blockers: Docker socket not available in this environment.

## 2026-07-17 02:04 — Gate ECR + App Runner IAM roles

- Author: Codex (Rushi)
- Service: ZEXVRO Gate
- Summary: Created ECR `zexvro-agent-auth` and IAM roles `zexvro-gate-apprunner-instance` (Dynamo+Secrets) and `zexvro-gate-apprunner-ecr-access`. Docker daemon unavailable — App Runner image not pushed.
- Files: `docs/iam/gate-apprunner-*.json`, `scripts/deploy-agent-auth-aws.mjs` (`--apply-ecr`, `--apply-all-infra`)
- Verification: role ARNs and ECR URI via AWS CLI; tests still 23/23.
- Follow-ups: docker build/push; create App Runner service; WebAuthn verify; rotate signing secret; De-pin payer middleware.
- Blockers: Docker socket/daemon not available in this session.

## 2026-07-17 02:09 — Gate WebAuthn hard path wired (register + assert)

- Author: Codex (Rushi)
- Service: ZEXVRO Gate
- Summary: Integrated @simplewebauthn/server. Registration options/verify routes; challenge complete accepts proofType=webauthn; credentials stored in memory/Dynamo repos. Tests 25+.
- Files: `webauthn.ts`, `challenges.ts`, `app.ts`, `repository*.ts`, deps package.json
- Decisions: **Accepted** — WebAuthn is production hard human path; session_pop remains soft; soft_confirm dev-only.
- Verification: lint/build clean; unit+integration tests for options origin allowlist.
- Follow-ups: Browser passkey UI; App Runner docker push; rotate secret; De-pin payer.
- Blockers: Docker daemon still unavailable for image push.

## 2026-07-17 02:14 — Gate Cognito admin + De-pin bind helper

- Author: Codex (Rushi)
- Service: ZEXVRO Gate
- Summary: Admin routes protected with optional Cognito access-token middleware (default on in production). De-pin payer allowlist helper + tests. 31 tests green.
- Files: `adminAuth.ts`, `config.ts`, `app.ts`, `depinBind.ts`, packages depin-bind.js, `.env.example`
- Decisions: **Accepted** — production admin auth on by default; local/dev open for demo-keys DX.
- Verification: lint/build/test 31/31.
- Follow-ups: App Runner docker push; browser passkey UI; Nabil wires assertPayerAllowed on De-pin edge; rotate signing secret.
- Blockers: Docker daemon unavailable for image build/push.

## 2026-07-17 02:17 — De-pin optional Gate capability + payer bind

- Author: Codex (Rushi + coordination with Nabil ownership of De-pin)
- Service: De-pin (composition only) + Agent Auth
- Summary: Added optional Access Shield plane-1 capability verification to De-pin without embedding a classifier. Per-provider requireCapability; optional payer allowlist bind after x402 authorize. De-pin tests 38; agent-auth 31.
- Files: `services/depin/src/capabilityGate.ts`, `proxy.ts`, `config.ts`, `domain.ts`, `capabilityGate.test.ts`, README
- Decisions: **Accepted** — De-pin calls Gate verify remotely; does not reimplement classification. Payer bind fail-closed with 403.
- Verification: depin lint/build/test green; agent-auth tests green.
- Follow-ups: App Runner Gate deploy; e2e smoke Gate+Depin with real agent payment; rotate Gate secret.
- Blockers: Docker daemon unavailable for Gate image push.

## 2026-07-17 02:19 — Gate CodeBuild deploy blocked by account limit

- Author: Codex (Rushi)
- Service: ZEXVRO Gate
- Summary: Added `scripts/deploy-agent-auth-codebuild.mjs` (CodeBuild→ECR→App Runner, no local Docker). Source staged to S3; project `zexvro-agent-auth-build` created. **StartBuild failed**: `AccountLimitExceededException: Cannot have more than 0 builds in queue for the account` (same limit noted in docs/aws_inventory for nft-depin-build).
- Files: `scripts/deploy-agent-auth-codebuild.mjs`, package script `gate:deploy`
- Verification: agent-auth 31 tests; depin 38 tests; Dynamo ACTIVE; ECR+IAM exist.
- Follow-ups: Raise CodeBuild concurrent limit OR enable local Docker for push; then App Runner create; set AGENT_AUTH_ISSUER; rotate secret; e2e health curl.
- Blockers: CodeBuild concurrent builds = 0; Docker daemon inactive.

## 2026-07-17 02:35 — Gate P0 security + SDK DX (post dual-agent critique)

- Author: Codex (Rushi)
- Service: ZEXVRO Gate (`services/agent-auth`, `packages/agent-auth-sdk`)
- Summary: Past 2:00 IST objective window; continued production Gate work. Security explorer + DX explorer critiques integrated. Implemented atomic challenge complete (memory + Dynamo CAS), agent client/agent key equality, honest ceremony_strength conf, human maxReuse=1, request-bound Pop fields, optional requireHumanPop, SDK soft_confirm guard + gateFetch + middleware requirePop wiring. Updated context.md §4 Accepted decisions, PLAN_FINAL post-critique section, SDK README honesty. CodeBuild concurrent Linux/Small quota increase requested (PENDING, was 0). Docker still inactive; ECR empty; App Runner Gate not live. NFT + De-pin App Runner still RUNNING.
- Files: `challenges.ts`, `verify.ts`, `domain.ts`, `app.ts`, `repository.ts`, `repository.dynamo.ts`, `gate.test.ts`, `packages/agent-auth-sdk/src/{index,middleware,browser}.js`, README, `docs/agent_auth_PLAN_FINAL.md`, `context.md`
- Decisions:
  - **Accepted** — Human soft JWT is bearer after mint; mitigate with maxReuse=1 + optional requireHumanPop; do not overclaim anti-relay.
  - **Accepted** — Origin must pass expectedHtm/Htu/bodyHash for true request-bound Pop.
  - **Accepted** — conf is ceremony_strength not ML detection.
- Verification: agent-auth **33/33** tests; depin **38/38**; lint clean.
- Follow-ups: Wait CodeBuild quota approval → `npm run gate:deploy`; rotate signing secret; dashboard CRUD; npm publish path; SEP-10 optional; human Pop default product decision.
- Blockers: CodeBuild concurrent builds = 0 until quota approved; Docker daemon inactive.


## 2026-07-17 02:52 — Gate local completion (no AWS deploy)

- Author: Codex (Rushi)
- Service: ZEXVRO Gate
- Summary: Local-only goal finish work. Enriched demo-keys (projectId, allowedOrigins), GET /v1/admin/agents + dashboard register/list, GateProtect + live keys/status/security strip, smoke covers soft + session_pop + agent PoP + list agents, gate:protect-demo for any action, quickstart + developer guide + service README. Frontend deps installed; production vite build succeeds (includes Gate UI). **No App Runner/ECR deploy.**
- Verification: agent-auth 33/33; depin 38/38; lint agent-auth clean; smoke OK; protect-demo checkout/search/index OK; frontend `npm run build` OK. Frontend `tsc --noEmit` has pre-existing testing-library type errors unrelated to Gate.
- Follow-ups (after user local test): AWS deploy only with explicit approval; optional fix frontend RTL types.
- Blockers: none for local testing.

## Progress 2026-07-17 · Self-hosted multi-type CAPTCHA (Gate)

- Owner: Rushi / Wraient (Agent Auth / Gate)
- Summary: Implemented self-hosted 8-type human CAPTCHA as first-class Gate ceremony (`proofType: captcha_pass`). Assets are locally generated SVG (no CDN). Agent path unchanged (Ed25519 + PoP). Sample third-party site demo at `GET /demo/captcha`. SDK: `@zexvro/gate/captcha` + BrowserGate `mode: 'captcha'`.
- Files:
  - `services/agent-auth/src/captcha/{types,assets,engine,service,demoPage}.ts`
  - `services/agent-auth/src/{domain,challenges,app,stores}.ts`
  - `packages/agent-auth-sdk/src/{captcha.js,browser.js}`
  - `frontend/src/components/gate/GateProtect.tsx` (default mode captcha)
  - `docs/agent_auth_captcha.md`
- Tests: `npm run test:agent-auth` → **43 passed** (engine + integration: mint human, reject agent captcha_pass, demo page).
- Local verify: `npm run dev:agent-auth` then open http://localhost:4103/demo/captcha
- Out of scope still: AWS deploy, photoreal image bank, captcha farm resistance claims.
- Decisions: Captcha mints **human only**; answers never leave server; conf≈0.55 honesty; audio uses browser speech synthesis + server digit check.

## Progress 2026-07-17 · Captcha modal + photo bank

- Summary: Human captcha UX is now a fixed **360×480 modal popup** (SDK default `mode: 'modal'`) so host pages do not reflow. Image tiles load from local bank `services/agent-auth/captcha-assets/` (~92 files across 12 labels; Wikimedia/Openverse seeds + local generated rasters). Engine prefers photo bank, falls back to SVG icons.
- Files: `packages/agent-auth-sdk/src/captcha.js`, `browser.js`, `services/agent-auth/src/captcha/{assets,demoPage,engine}.ts`, `captcha-assets/**`, `GateProtect.tsx`
- Verify: `npm run test:agent-auth` 43 green; demo http://localhost:4103/demo/captcha

## Progress 2026-07-18 · Premium captcha UX
- Soft gate, hero types, attempts chip, co-brand, animations, a11y focus trap/live regions, mobile dvh shell.
- Files: packages/agent-auth-sdk/src/captcha.js, captcha/engine.ts, demoPage.ts, app.ts, service.ts
- Tests: 50 green. Demo: /demo/captcha

## Progress 2026-07-18 · Success animation + photo curation UI
- Premium stroke-draw verified animation (no emoji) in SDK + demo (~720ms).
- New local curator: GET /demo/curate — pick 50 images/label from Kaggle pools; save to captcha-assets/verified; Publish to bank.
- Files: captcha.js playSuccess, curate.ts, curatePage.ts, app routes, assets prefers verified manifest.
- Tests 51 green.

## Progress 2026-07-18 · Verified-only bank + 4 new human captcha types

- Author: Codex (Rushi / Wraient)
- Service: ZEXVRO Gate / agent-auth captcha
- Summary: Deleted unverified live label dirs + thin hydrant; photo bank is **verified only** (442 photos / 12 labels). Added human captcha types: `count_objects`, `photo_rotate`, `binary_pick`, `majority_select` (engine + scoring + SDK modal + demo page + tests). Publish paths no longer copy live root banks. Docs updated.
- Files: `services/agent-auth/src/captcha/{types,engine,assets,demoPage,curate,engine.test}.ts`, `packages/agent-auth-sdk/src/captcha.js`, `scripts/curate_batch.py`, `docs/agent_auth_captcha.md`, `captcha-assets/{verified,SOURCES.md}`
- Decisions:
  - **Accepted** — Verified bank is sole photo source (`verified/manifest.json`); no SVG tiles for image challenges.
  - **Accepted** — New types stay human-only; agents remain Ed25519 + PoP.
  - **Accepted** — count_objects allows off-by-one when count ≥ 3 (wiggly human scoring).
- Verification: `npm run test:agent-auth` → **56 passed**; local issue smoke for all 15 types OK.
- Local: `npm run dev:agent-auth` → http://localhost:4103/demo/captcha
- Out of scope: AWS deploy, better datasets until user asks.

## Progress 2026-07-18 · Drop weak captcha types (math_visual, audio)

- Author: Codex (Rushi)
- Service: ZEXVRO Gate captcha
- Summary: Removed `math_visual` (poor UX) and `audio`/Speak (no real audio backend yet). CAPTCHA_TYPES now **13** human visual/interaction types on verified photos + text/rotate/slider.
- Verification: `npm run test:agent-auth` → 55 passed.

## Progress 2026-07-18 · Captcha images object-fit contain

- Author: Codex (Rushi)
- Service: Gate captcha UI
- Summary: Image tiles, reference thumbs, and label_pick hero use `object-fit: contain` (not cover) on dark frames so subjects are fully visible for identification. Letterboxing preferred over cropping.
- Files: `demoPage.ts`, `packages/agent-auth-sdk/src/captcha.js`
- Verify: computed style objectFit=contain; agent-auth tests green.

## Progress 2026-07-18 · Captcha security hardening (post dual-explorer)

- Author: Codex (Rushi)
- Service: ZEXVRO Gate captcha
- Summary: Dual security explorers found critical free-solves (rotate displayDegrees, slider SVG target, unlimited re-issue, preferredType force, text SVG scrape). Hardened engine/service/complete/assets; curate writes disabled in prod; tests cover free-solve rejection + re-issue limits + siteKey assets.
- Key fixes: relative-only rotate; slider track without secret; bitmap text; no class names in prompts; exact scoring; max 3 issues/challenge; captchaId-required complete; siteKey-required assets.
- Verification: `npm run test:agent-auth` → **61 passed**.
- Honesty: still not farm-proof / not request-bound.
